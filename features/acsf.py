...
def acsf(gips,functional,form,rcut,parameters):
    """
    Input
    -----
    ...
    for _gip in gips:
        for _s in _gip:
            ...
            # need atomic number
            ultraz = np.asarray([parsers.atomic_data.atomic_number.atomic_number[_s_] for _s_ in ultra_species])
            localz = np.asarray([parsers.atomic_data.atomic_number.atomic_number[_s_] for _s_ in _s["species"]])
            ...
            for atom,_r in enumerate(np.dot(_s["positions"],_s["cell"])):
                ...
                if form.lower() == 'isotropic':
                    if category == 'behler':
                        ...
                        structure_feature[atom] = np.sum(taper1(x=dr,rcut=rcut,fs=fs)*normal(x=dr,mu=mu,Lambd=Lambd)*\
                                atomic_num)*zatom
